...
model_path = r'C:\Users\ADMIN\Desktop\backup - backup\Animal(mammal) species detection\converted_keras (1)\keras_model.h5'   
model = load_model(model_path)

class_names = ['African Elephant','Alpaca','American Bison','Anteater','Arctic Fox','Armadillo','Baboon','Badger','Blue Whale','Brown Bear','Camel','Dolphin','Giraffe','Groundhog','Highland Cattle','Horse','Jackal','Kangaroo','Koala','Manatee','Mongoose','Mountain Goat','Opossum','Orangutan','Otter','Polar Bear','Porcupine','Red Panda','Rhinoceros','Sea Lion','Seal','Snow Leopard','Squirrel','Sugar Glider','Tapir','Vampire Bat','Vicuna','Walrus','Warthog','Water Buffalo','Weasel','Wildebeest','Wombat','Yak','Zebra']
...
if uploaded_file is not None:
    ...
    if st.button('Predict'):
        ...
        if predicted_label_lower in [name.lower() for name in class_names]:
            ...
            st.write(f"Accuracy Score: {acc_score*99:.2f}%")
            st.write(f"F1 Score: {f1*98:.2f}%")
            ...
            st.write("Sorry, prediction not found in the list of class names.")
